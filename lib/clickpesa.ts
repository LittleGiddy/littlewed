// lib/clickpesa.ts - API INTEGRATION
...
export async function getAccessToken(): Promise<string> {
  try {
    ...
    // ✅ API Integration uses the same /generate-token endpoint
...
export async function generateCheckoutLink(params: {
  ...
  try {
    ...
    // ✅ API Integration uses the same payload format
    const payload = {
    ...
    // ✅ API Integration uses the same endpoint
