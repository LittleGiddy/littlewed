...
export default async function EventsPage() {
  const session = await getServerSession(authOptions) // ✅ now authOptions is available
  if (!session) redirect('/login')

  const role = (session.user as any).role // ✅ type cast
  const tenantId = (session.user as any).tenantId // ✅ use tenantId, not userId

  // ✅ verify user has access
  if (!['CLIENT', 'SUPER_ADMIN', 'STAFF'].includes(role)) {
    redirect('/login')
  }
  ...
  const events = await prisma.event.findMany({
    where: { tenantId }, // ✅ changed from userId to tenantId
    orderBy: { date: 'asc' },
  })
