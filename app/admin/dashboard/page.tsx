...
export default async function AdminDashboard() {
  ...
  // ✅ Robust check: session must exist and role must be SUPER_ADMIN
  ...
  // Logo component (inline SVG or image)
  const Logo = () => (
    ...
    <div style={{ minHeight: '100vh', fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .manage-link:hover { opacity: 0.7; }
        @media (max-width: 768px) { .admin-page { padding: 16px; } .stats-grid { gap: 12px; } .stat-card { padding: 16px; } .stat-icon { width: 44px; height: 44px; } .stat-number { font-size: 24px; } .tenant-table th, .tenant-table td { padding: 12px 16px; } .table-header { padding: 14px 16px; } }
      `}</style>
          ...
          <div className="table-wrapper">
            ...
            <div className="overflow-x-auto">
              <table className="tenant-table">
                <thead><tr><th>Name</th><th>Subdomain</th><th>Plan</th><th>Status</th><th>Users</th><th></th></tr></thead>
                <tbody>
                  {tenants.map(tenant => (
                    <tr key={tenant.id}>
                      ...
                      <td><span className={`status-badge ${tenant.subscriptionStatus === 'active' ? 'status-active' : 'status-inactive'}`}>{tenant.subscriptionStatus === 'active' ? 'Active' : 'Inactive'}</span></td>
                      <td>{tenant.users.length}</td>
                      <td><Link href={`/admin/tenants/${tenant.id}/manage`} className="manage-link">Manage <ArrowRight size={12} /></Link></td>
                    </tr>
                  ))}
                  {tenants.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', padding: '48px', color: '#9BAAB8' }}>No organisations yet. Create the first one.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
