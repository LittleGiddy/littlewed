...
import { 
  Key, Loader2, CheckCircle, XCircle, User, Users, 
  ArrowLeft, Camera, AlertCircle, Scan,
  Calendar, MapPin, Trash2, 
  Search, UserCheck,
  CheckCheck, PartyPopper
} from 'lucide-react';
  ...
  const processCheckin = async (value: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const welcomeSub = isFullyCheckedIn && maxCheckIns > 1
          ? `Fully checked in (${currentCount}/${maxCheckIns})`
          : maxCheckIns > 1
            ? `Checked in (${currentCount}/${maxCheckIns})`
            : undefined;
